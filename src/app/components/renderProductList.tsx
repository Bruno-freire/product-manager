    ...
    <div className="w-full bg-white p-4 rounded-xl shadow-sm border border-gray-100">
      <div className="flex items-start justify-between">
        <h3 className="font-semibold text-gray-800">
          {title} <span className="text-sm text-gray-500 font-medium">({filteredProducts.length})</span>
        </h3>
        ...
        {filteredProducts.length > 0 ? (
          <ul className="space-y-2">
            {filteredProducts.map(({ name, code, amount, entryDate }) => {
              ...
              const isZero = typeof amount === "string" && amount.trim() === "0";
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{formatDuration(entryDate)}</div>
                    </div>
                  </div>
          ...
          <p className="text-gray-500">No {title.toLowerCase()} products found</p>
        )}
      </div>
    </div>
  );
